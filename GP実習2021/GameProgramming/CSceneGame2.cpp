...
void CSceneGame2::Init() {
	...
	mFadeout = 0.0;
	mFadein = 1.0;
	CScore::mScore = CSceneGame::mScorestore;

	TextureChara1.Load("resource\\ch101.tga");
	Texture2.Load("resource\\background.tga");//背景
...
void CSceneGame2::Update() {
	...
	if (CPlayer::spInstance->mRect.x >= 800){
		...
		if (mFadeout >= 1.0){
			mScene = EGAMEBOSS;
			CSceneGame::mCharaHp1store = CPlayer::mChara1Hp;
			CSceneGame::mScorestore = CScore::mScore;
